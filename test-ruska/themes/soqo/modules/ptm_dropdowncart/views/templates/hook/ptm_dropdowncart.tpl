...
<div id="_desktop_cart" class="blockcart_container">
  <div class="ptm_blockcart blockcart cart-preview {if $cart.products_count > 0}active{else}inactive{/if} {if isset($dropdownonphones) && $dropdownonphones}display_mobile_cart{/if}" data-refresh-url="{$refresh_url}">
    <div class="header">
    ...
    <div class="ptm_blockcart_body {if isset($display_shadow) && $display_shadow == 2}noshadow{elseif isset($display_shadow) && $display_shadow == 3}lightshadow{elseif isset($display_shadow) && $display_shadow == 4}mediumshadow{elseif isset($display_shadow) && $display_shadow == 5}strongshadow{/if}">
      ...
      <div class="checkout-link">
        <a class="button_order_cart btn btn-default btn btn-primary" href="{$option_url}">
          {if $redirect_visitors_opt == 'cart'}
              {l s='Shopping cart' mod='ptm_dropdowncart'} <i class="fa fa-paper-plane"></i>
          {else}
              {l s='Checkout' mod='ptm_dropdowncart'} <i class="fa fa-paper-plane"></i>
          {/if}
        </a>
      </div>
    </div>
